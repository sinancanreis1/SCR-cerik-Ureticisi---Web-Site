					...
					<div class="col-12 col-md-4">
						<div class="p-4 rounded-4" style="background: #ffffff; border: 1px solid #e5e7eb; box-shadow: 0 4px 15px rgba(0,0,0,0.04);">
							...
							<ul class="list-unstyled mb-0" style="font-size: 1.05rem; line-height: 2; color: #565960;">
								...
								<li><strong style="color: #030712;">Tarih:</strong> {{ $product->created_at ? $product->created_at->format('Y') : '2026' }}</li>
							</ul>
						</div>
					</div>
				</div>
									...
									<div class="card portfolio-item layout-2 scale has-shadow">
										<div class="image-holder">
											<a class="card-thumb" href="{{ route('proje.detay', $relProduct->slug) }}">
												@if($relProduct->image)
													<img src="{{ Storage::url($relProduct->image) }}" alt="{{ $relProduct->title }}">
												@else
													<img src="{{ asset('frontend-assets/img/content/case-1.jpg') }}" alt="{{ $relProduct->title }}">
												@endif
											</a>
										</div>
